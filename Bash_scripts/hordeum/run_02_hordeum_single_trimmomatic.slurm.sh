...
OUTDIR=/nfs/pgsb/projects/comparative_triticeae/phenotype/flower_development/refsets/hordeum/02_Trimmomatic_single
...
IN="$INPATH/${array[0]}.fastq.gz"
        ...
        cd $OUTDIR
        date > $STARTED
        java -jar /nfs/pgsb/commons/apps/trimmomatic/Trimmomatic-0.35/trimmomatic-0.35.jar SE -threads 4 $R ${rr}_trimmed.fastq.gz ILLUMINACLIP:/nfs/pgsb/data/grill-rcar/02_Trimmomatic/adapters/adapters_et_al.nr.fasta:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:20 MINLEN:60 > $LOG 2>&1

        date > $DONE



# sbatch -N 1 ~/scripts/hordeum/run_02_hordeum_single_trimmomatic.slurm.sh
